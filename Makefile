...
KIND_CLUSTER ?= fleet-manager-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name fleet-manager-builder
	$(CONTAINER_TOOL) buildx use fleet-manager-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm fleet-manager-builder
	rm Dockerfile.cross

.PHONY: build-installer
build-installer: kustomize ## Generate a consolidated YAML with the deployment.
	mkdir -p dist
